...
import { MarketPokemon, MarketFilter, PokemonClass, BonusType, PokemonType, PokemonGroup, GameItem } from '../types';
import { Button } from './ui/Button';
import { CLASS_COLORS, TYPE_COLORS, GAME_ITEMS } from '../constants';
...
export const Marketplace: React.FC<MarketplaceProps> = ({ 
  ...
  }, [targetTime, loading, onRefresh, filter, activeTab]);
  ...
  return (
    <div className="flex flex-col gap-6">
       <div className="bg-slate-800/80 p-6 rounded-2xl border border-white/5 shadow-xl">
         <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
            ...
            <div className="flex gap-2 bg-slate-900 p-1 rounded-lg">
                <button 
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'pokemon' ? 'bg-violet-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                >
                    Pokémon
                </button>
                <button 
                    onClick={() => setActiveTab('items')}
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'items' ? 'bg-amber-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                >
                    Items
                </button>
            </div>
         ...
         {/* Advanced Filter UI (Only for Pokemon) */}
         {activeTab === 'pokemon' && (
            <>
